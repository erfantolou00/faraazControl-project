...
import { ArrowLeft, ArrowRight, CheckCircle2, Factory, ShieldCheck, Zap } from "lucide-react";
...
export default function HeroSection({ data, locale }: HeroSectionProps) {
  ...
  const badges = isRtl
    ? ["تابلو برق صنعتی", "تست FAT", "اتوماسیون و کنترل"]
    : ["Industrial panels", "FAT testing", "Automation and control"];
  ...
  return (
    ...
    <div className="absolute inset-0">
      ...
      {/* Overlay تیره‌تر برای هماهنگی با تم زرد + مشکی */}
      <div
    ...
    <div className="container relative z-10 flex min-h-[92vh] items-center px-6 py-28 lg:px-10">
      ...
      <div className="absolute bottom-8 left-6 right-6 hidden items-center justify-between border-t border-white/15 pt-5 text-sm text-white/70 lg:flex">
        <span>
          {isRtl
            ? "طراحی برای ایمنی، دوام و بهره‌برداری پایدار"
            : "Designed for safety, durability, and stable operation"}
        </span>
        <span className="inline-flex items-center gap-2">
          <ShieldCheck className="h-4 w-4 text-primary" />
          IEC-minded engineering
        </span>
      </div>
    </div>
  </section>
